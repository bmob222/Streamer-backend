...
extension TMDBProvider {
    public func convert(_ movie: TMDb.Movie) -> MediaContent? {
        ...
        guard let posterURL = imagesConfiguration?.posterURL(for: movie.posterPath, idealWidth: 300) else {
            return nil
        }
    ...
    public func convert(_ tvseries: TVSeries) -> MediaContent? {
        ...
        guard let posterURL = imagesConfiguration?.posterURL(for: tvseries.posterPath, idealWidth: 300) else {
            return nil
        }

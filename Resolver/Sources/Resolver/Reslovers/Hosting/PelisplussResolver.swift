    ...
    func getMediaURL(url: URL) async throws -> [Stream] {
       let headers = [
        ...
        if url.absoluteString.contains("embed.php") {
            ...
            let streams = await Utilities.extractURLs(content: html).asyncMap {
        ...
        if let h = url.queryParameters?["h"] {
            ...
            let (_, rResponse)  = try await ResolverURLSession.shared.session.asyncData(for: rRequest)
            return try await HostsResolver.resolveURL(url: rResponse.url!)
        }

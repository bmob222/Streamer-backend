...
struct ShowBoxResolver: Resolver {
    let name = "ShowBox"
    static let domains: [String] = ["showbox-free.host", "showbox-free.host"]
    @EnviromentValue(key: "subtitles_srt_url", defaultValue: URL(staticString: "google.com/"))
    var subtitlesBaseURL

    func canHandle(url: URL) -> Bool {
        Self.domains.firstIndex(of: url.host!) != nil || url.host?.contains("showbox-free") == true
    }
    ...
    func getMediaURL(url: URL) async throws -> [Stream] {
        ...
        return try await content.data.list.concurrentMap { item -> Stream? in
            ...
            var name = item.quality == "org" ? "ShowBox Original" : "ShowBox"

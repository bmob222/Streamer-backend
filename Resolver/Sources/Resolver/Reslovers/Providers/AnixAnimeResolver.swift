...
struct AnixAnimeResolver: Resolver {
    let name = "Anix"
    static let domains: [String] = ["anix.to"]
    private let baseURL: URL = URL(staticString: "https://anix.to/")
    ...
    func getMediaURL(url: URL) async throws -> [Stream] {
        ...
        return try await ["vidplay", "mycloud", "filemon", "mp4upload"].concurrentMap { server -> [Stream]?  in
            let watchURL = consumetURL.appendingPathComponent("anime/")

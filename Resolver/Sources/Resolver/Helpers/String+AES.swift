...
public extension String {
    ...
    func matchesDotMatchesLineSeparators(for regex: String) -> [String] {
        do {
    ...
    func adilbo_deobfescate() -> String {
        let t_script = self.matchesDotMatchesLineSeparators(for: "<script.*?;.*?'(.*?);")
        let t_int = self.matchesDotMatchesLineSeparators(for: "/g.....(.*?)\\)")
        if !t_script.isEmpty && !t_int.isEmpty {
            var script = t_script[0].replacingOccurrences(of: "'", with: "")
            ...
            for elm in sc {
                ...
                if !t_ch.isEmpty {
                    ...
                    page = page +  String(UnicodeScalar(nb)!)
                }
            }
            return page
        } else {
    ...
    func percentEncoding() -> String {
        ...
        var result = self.addingPercentEncoding(withAllowedCharacters: charSet) ?? ""
